...
const TeamAtCspComponent = (props) => {
  ...
  const teamDataUnfiltered = teamDataQuery.allContentfulIndividualTeamMember.nodes;
      ...
      <div className="overflow-hidden">
        <div className="flex justify-start flex-shrink-0" id="teamSlider">
          {teamData.map((iteam1, index) => {
            return <IndividualInvestmentComponent presentId={`${zeroOrNot(index)}` + (index + 1)} customer={iteam1.positionInTheCompany} company={iteam1.nameOfTheEmployee} endId={teamData.length} handleNext={handleNext} handlePrevious={handlePrevious} key={index} slug={iteam1.slug} fact="hide" logoImage={iteam1.pictureOfTheEmployee.fluid} />;
          })}
        </div>
        <PreviousNextButtonComponent handleNext={handleNextInDesktop} handlePrevious={handlePrevious} />
      </div>
    </div>
    </Layout>
  );
