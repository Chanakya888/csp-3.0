...
const IndividualInvestmentComponent = (props) => {
	...
	useEffect(() => {
		if (props.AnimateComponents === 'true') {
			...
			t1.staggerFrom(document.querySelectorAll(`.${props.type}-line_innerDiv`), 1, { y: '100%', ease: Power4.easeOut }, 0.15);
			//bringing up about lines
			t1.from(aboutLines, 0.75, { opacity: 0, y: 20, ease: Power4.easeOut }, '-=2');
			//removing overlay
			t1.to(
				`#${props.type}-overlay`,
				{
	...
	return (
		<div id={`${props.type}`} className={`w-full flex-shrink-0 ${widthOfComponent} md:pr-${PaddingRightForComponentOrPage}`}>
			<p className="subtitle-text investment-customer-text" id={`${props.type}-subtitle`}>
				{props.customer}
			</p>
			...
			<div className={`height-of-des-div pt-6 w-11/12 md:w-full xl:w-11/12 ${hideDes}`} id={`${props.type}-des`}>
				<p>{props.companyDes}</p>
			</div>

			<div className={`pt-${paddingInTeamComponent} md:pt-${paddingTopInTeamComponent} pl-${paddingLeftInTeamComponent} xl:pl-0 `}>
            ...
            >{`/${zeroOrNot}${props.endId}`}</h2> */}
				...
				<div className="mt-5 xl:mt-8 xl:mr-6 relative">
					<Img id="investmentImage" className="investment-image" fluid={props.logoImage} />
					<div className={`investment-rectangle ${hideInNonAnimatedPaged}`} id={`${props.type}-overlay`} />
				</div>
